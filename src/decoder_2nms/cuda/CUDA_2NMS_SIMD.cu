...
__global__ void LDPC_Sched_Stage_1_2NMS_SIMD(unsigned int var_nodes[_N],
	...
	//
	{
		...
		for (int z = 0; z < DEG_1_COMPUTATIONS; z++) {
			...
			t_min1.v = min1;
			t_min2.v = min2;
			t_min1.c[0] = (((float)t_min1.c[0]) * 0.750f);
			t_min1.c[1] = (((float)t_min1.c[1]) * 0.750f);
			t_min1.c[2] = (((float)t_min1.c[2]) * 0.750f);
			t_min1.c[3] = (((float)t_min1.c[3]) * 0.750f);
			t_min2.c[0] = (((float)t_min2.c[0]) * 0.875f);
			t_min2.c[1] = (((float)t_min2.c[1]) * 0.875f);
			t_min2.c[2] = (((float)t_min2.c[2]) * 0.875f);
			t_min2.c[3] = (((float)t_min2.c[3]) * 0.875f);
			const unsigned int cste_1 = t_min2.v;//vminu4(vsubus4(min2, 0x01010101), 0x1F1F1F1F);
			const unsigned int cste_2 = t_min1.v;//vminu4(vsubus4(min1, 0x01010101), 0x1F1F1F1F);
