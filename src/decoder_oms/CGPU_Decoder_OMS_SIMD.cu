...
#include "CGPU_Decoder_OMS_SIMD.h"
...
CGPU_Decoder_OMS_SIMD::CGPU_Decoder_OMS_SIMD(size_t _nb_frames, size_t n, size_t k, size_t m):
...
*/
	...
	cudaFuncGetAttributes(&attr, LDPC_Sched_Stage_1_OMS_SIMD);
...
CGPU_Decoder_OMS_SIMD::~CGPU_Decoder_OMS_SIMD()
{
}

void CGPU_Decoder_OMS_SIMD::initialize()
{
}


void CGPU_Decoder_OMS_SIMD::decode(float Intrinsic_fix[_N], int Rprime_fix[_N], int nombre_iterations)
{
    cudaError_t Status;
    ...
    LDPC_Sched_Stage_1_OMS_SIMD<<<nb_blocks, BLOCK_SIZE>>>((unsigned int*)device_V, (unsigned int*)d_MSG_C_2_V, d_transpose, nombre_iterations);
